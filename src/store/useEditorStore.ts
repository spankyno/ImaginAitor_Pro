...
export const useEditorStore = create<EditorState>((set, get) => {
  ...
  return {
    ...
    addFiles: async (files) => {
      for (const file of files) {
        try {
          ...
          set((s) => ({
            docs: { ...s.docs, [doc.id]: { doc, past: [], future: [], previewUrl: null, previewWidth: width, previewHeight: height, isRendering: false, renderError: null } },
            order: [...s.order, doc.id],
            activeId: s.activeId ?? doc.id,
          }))
          get().rerender(doc.id, { immediate: true })
        } catch (e) {
          const classified = classifyError(e)
    ...
    removeDoc: (id) => set((s) => {
      const ds = s.docs[id]
      if (ds) {
        // Release both the pristine source bitmap and the last preview
        // blob URL — without this, closing/switching images silently
