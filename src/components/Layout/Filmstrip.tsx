...
function ThumbCell({ columnIndex, style, ids }: CellComponentProps<CellProps>) {
  ...
  const thumbUrl = useEditorStore((s) => s.docs[id]?.thumbUrl)
  ...
  if (!id) return null
  // Thumbnails stream in from their own worker queue and usually resolve
  // first (they're tiny); fall back to the full preview if that happens to
  // be ready sooner (e.g. for the already-active image).
    ...
    <div style={style} className="flex items-center justify-center">
      ...
      >
        {imgSrc ? (
          ...
          <div className="h-full w-full animate-pulse bg-bg-panel-2" aria-hidden="true" />
        )}
        <span
          role="button"
          tabIndex={-1}
